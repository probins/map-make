...
var registry = 'registry/',
    ...
    errors = {
      ...
      noMap: function(err) {
        errors.error(err, 'Unable to load olMap');
        return;
      },
...
if (!mapDef && !qsOptions) {
  ...
  System.fetch({address: htmlDir + 'initialForm.html', metadata: {}})
  ...
  // if mapDef, fetch it
  var getMapDef = function() {
    if (mapDef) {
      return System.fetch({address: decodeURIComponent(mapDef), metadata: {}});
    } else {
  ...
  getMapDef().then(function(mapDefined) {
    if (mapDefined) {
      mapDefined = JSON.parse(mapDefined);
    }
