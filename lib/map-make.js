...
System.paths.projfile = 'https://cdnjs.cloudflare.com/ajax/libs/proj4js/1.1.0/proj4js-compressed.js.js';
System.map.proj = 'projfile!' + pluginDir + 'projWrapper';
...
if (!mapDef && !qsOptions) {
  ...
  // no need for 'then' so no need for Promise
    getModule({name: htmlDir + 'initialForm.html!jspm:text'})
      .then(displayForm, errorHandler);
  ...
  // first import olMap
  getModule({name: 'olMap'}).then(function(olMapIn) {
    // System.import('jspm:css').then(function(css) {
    //   System.fetch({address: '../modular-ol3/dist/olCustom.css', metadata: {plugin: css}});
    // }, errorHandler);
    olMap = olMapIn;
    // ... and optionally mapDef
    if (mapDef) {
      var encoded = decodeURIComponent(mapDef);
      System.paths.myjson = encoded + '.js';
      return getModule({name: 'myjson!jspm:json'});
    }
  }, errorHandler).then(function(mapDefined) {
    ...
    if (options.rasters) {
      ...
      for (i = 0; i < options.rasters.length; i++) {
        ...
        imports.push({name: sourceDir + modName});
      }
  ...
  }, errorHandler).then(function(importeds) {
      ...
      for (i = 0; i < options.vectors.length; i++) {
        if (options.vectors[i].styles) {
          for (var j = 0; j < options.vectors[i].styles.length; j++) {
            ...
            styles.push(style);
            imports.push({name: styleDir + style + '.json!jspm:json'});
          }
        }
      }
    ...
    if (options.widgets) {
      for (var widget in options.widgets) {
        getModule({name: widgetDir + widget});//.catch(errorHandler);
      }
    }
...
function getModule(options) {
  ...
  // options can be module name and address/url
  // return a Promise where resolve/reject are success/fail on import
  return new Promise(function(resolve, reject) {
    var address = options.address ? {address: options.address} : null;
    System.import(options.name, address).then(resolve, reject);
  });
}
