...
// and if no options, will display empty map with default 4326 view
    ...
    System.import('olMap').then(function(olMapIn) {
      ...
      if (options.rasters) {
        olMap.addInitialRasters(importeds, options);
      }

      //   var imports = [];
      //   // assemble array of style json files to import
      //   options.vectors.forEach(function(v) {
      //     if (v.styles) {
    ...
    // }, errors.rasters).then(function(importeds) {
      if (options.vectors) {
        ...
        //   var styleDefs = {};
        //   importeds.forEach(function(imp, i) {
        //     styleDefs[styles[i]] = JSON.parse(imp);
        //   });
        //   options.vectors.forEach(function(v) {
        //     if (v.styles) {
        //       v.styles.forEach(function(s) {
        ...
        olMap.addInitialVectors(options);
      }
