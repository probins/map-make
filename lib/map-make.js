  ...
  getModule('olMap').then(function(olMapIn) {
    ...
    if (options.rasters) {
      ...
      for (i = 0; i < options.rasters.length; i++) {
        ...
        olMap.addRaster(importeds[i], options, apikey, i === 0); // #0 is active
      }
    ...
    if (options.vectors) {
      ...
      if (styles) {
        for (i = options.rasters.length; i < importeds.length; i++) {
          styleDefs[styles[i-options.rasters.length]] = importeds[i];
        }
        for (i = 0; i < options.vectors.length; i++) {
          if (options.vectors[i].styles) {
            for (var j = 0; j < options.vectors[i].styles.length; j++) {
      ...
      for (i = 0; i < options.vectors.length; i++) {
        olMap.addVector(options.vectors[i]);
      }
