...
System.paths.layerswitcher = utils.componentDir + 'layerswitcher.js';
System.paths.toolbar = utils.componentDir + 'toolbar.js';
...
// and if that doesn't exist either, will display empty map with default 4326 view
    ...
    System.import('olMap').then(function(olMapIn) {
      ...
      if (options.rasters) {
        ...
        for (i = 0; i < options.rasters.length; i++) {
          ...
          imports.push(utils.sourceDir + modName);
        }
        ...
        for (i = 0; i < options.vectors.length; i++) {
          if (options.vectors[i].styles) {
            for (var j = 0; j < options.vectors[i].styles.length; j++) {
              ...
              styles.push(style);
              imports.push({address: utils.styleDir + style + '.json', metadata: {}});
            }
          }
        }
      ...
      if (options.widgets) {
        for (var widget in options.widgets) {
          System.import(utils.widgetDir + widget);//.catch(errorHandler);
        }
      }
      if (options.components) {
        for (var component in options.components) {
          utils.importComponent(component, 'lib/' + utils.componentDir + component + '.html');
        }
      }
      ...
      if (!(options.widgets && options.widgets.mouseposition)) {
        ...
        mouseButton.addEventListener('click', function() {
          System.import(utils.widgetDir + 'mouseposition');
...
function getMapDef() {
  if (mapDef) {
    ...
    return utils.httpGet(decodeURIComponent(mapDef));
  } else {
