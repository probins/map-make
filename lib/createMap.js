...
// uses jspm global
  ...
  // options can be defined in a mapDef or in querystring
  ...
  if (window.location.search) {
    ...
    if (!mapDef) {
      jspm.import(htmlDir + 'initialForm.html!text', function(html) {
        var status = document.getElementById('status');
        status.style.top = '5%';
        status.style.left = '5%';
        status.innerHTML = html;
        document.getElementById('mapDefForm').addEventListener('click', function() {
          var qs = 'rasters=' + document.getElementById('rasters').value; // should always be present
          if (document.getElementById('lat').value != '') {
            qs += '&lat=' + document.getElementById('lat').value;
          }
          if (document.getElementById('lon').value != '') {
            qs += '&lon=' + document.getElementById('lon').value;
          }
          if (document.getElementById('zoom').value != '') {
            qs += '&zoom=' + document.getElementById('zoom').value;
          }
