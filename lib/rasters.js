...
function switcherHandler(evt) {
  ...
  let target = document.getElementById(evt.target.htmlFor) || evt.target;
  if (!target.value) {
    return; // do nothing if not label or checkbox
  }
  // 'this' is map
  let layerChange = changeLayer.bind(this);
...
function changeLayer(newRaster) {
  ...
  // 'this' is map
  let layerArray = this.getLayers().item(0).get('layers').getArray();
  let oldLayers = layerArray.filter(function(l) {
    return l.activeLayer === true;
  });
  let newLayers = layerArray.filter(function(l) {
  ...
  let projec = newLayers[0].get('source').getProjection();
  let newProjCode = projec ? projec.getCode() : 'EPSG:3857';
  let vectorLayers = this.getLayers().item(1).get('layers');
  // 4326 if vectors only
  let defaultCode = (vectorLayers.get('length') === 0) ? '' : 'EPSG:4326';
  let oldProjCode = oldLayers[0] ? oldLayers[0].get('source').getProjection().getCode() : defaultCode;
  if (newProjCode != oldProjCode) {
    // new projection, so change view and reproject vectors
    let newExtent;
    if (oldProjCode !== '') {
      let oldExtent = this.get('view').calculateExtent();
...
function add(rasters, options) {
  ...
  rasters.forEach((raster, i) => {
    ...
    let projCode = raster.projCode || 'EPSG:3857';
    let views = olMap.views;
    if (!views[projCode]) {
      ...
      if (raster.resolutions) {
        ...
        let closest = raster.resolutions.reduce(function(prev, curr) {
    ...
    let div = switcher.addRasterDiv(layers[0]);
...
export default {
  addInitial: function(rasters, options) {
    add(rasters, options);
    let projCode = rasters[0].default.projCode || 'EPSG:3857';
