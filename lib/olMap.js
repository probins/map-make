...
var switcher = require('layerswitchermodule');
    ...
    if (vector.type == 'mongodb') {
      sourceOptions.loader = function(extent, resolution, projection) {
        mm.httpGet(url).then(function(result) {
          ...
          var mongoLayers = map.getLayers().getArray().filter(function(l) {
...
function addVectorWidgetButtons(widgets) {
  widgets = widgets || {};
  var button, popup = 'registry/widgets/popup';
  if (!widgets.popup && !System.get(popup)) {
  ...
  var tooltip = 'registry/widgets/tooltip';
  if (!widgets.tooltip && !System.get(tooltip)) {
    button = document.createElement('button');
    button.id = 'tooltipButton';
    button.innerHTML = 'Tooltip'; // FIXME English
    button.addEventListener('click', function() {
      System.import(tooltip);
      this.style.display = 'none';
    });
    document.getElementById('widgets').appendChild(button);
  }
