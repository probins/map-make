...
function switcherRasterHandler(evt) {
  ...
  // 'this' is map
  var layerChange = changeRasterLayer.bind(this);
...
function changeRasterLayer(newRaster) {
  ...
  if (newProjCode != oldProjCode) {
    ...
    this.setView(views[newProjCode]);
    if (oldProjCode !== '') {
...
function addRasters(rasters, options) {
  ...
  rasters.forEach(function(raster, i) {
    ...
    div.addEventListener('click', switcherRasterHandler.bind(map));
...
function addVectors(options) {
  ...
  vectors.forEach(function(vector) {
    ...
    rtns[0].addEventListener('click', switcherVectorHandler.bind(map));
