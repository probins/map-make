...
function switcherRasterHandler(evt) {
  ...
  if (newProjCode != oldProjCode) {
    ...
    for (i = 0; i < vectorLayers.length; i++) {
      var features = vectorLayers[i].getSource().getAllFeatures();
  ...
  for (var i = 0; i < vectors.length; i++) {
    ...
    var vectOpts = {
      source: new ol.source[format]({
        url: url,
        reprojectTo: map.getView().getProjection(), // FIXME yuk!
    ...
    vectOpts.styleFunction = getStyleFunction(style);
...
module.exports = {
  createMap: function(target, noKeyboardPan) {
    map = new ol.Map({
      // currently only canvas handles vectors
      // can comment this out with custom builds that only include canvas
