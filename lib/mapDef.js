...
function parseElement(mapdefElement) {
  let options = {};
  const children = mapdefElement.children;
  for (let i = 0; i < children.length; i++) {
    const pair = [children[i].localName, children[i].innerHTML];
  ...
  switch (pair[0]) {
    ...
    case 'r':
      ...
      rasters.forEach(r => {
        ...
        const p = r.indexOf('{') === -1 ? r : JSON.parse(decodeURIComponent(r));
    ...
    case 'v':
      ...
      vectors.forEach(v => {
        ...
        if (typeof v === 'string') {
          ...
          // object with url and optionally id/group/attribution/style/type
          out = {
            url: v.url
          };
          out.id = v.id || v.url;
