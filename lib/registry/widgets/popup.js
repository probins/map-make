// uses ol.sphere.NORMAL.haversineDistance() not in standard exports
var ol = require('ol');
...
map.on(['singleclick'], function(evt) {
  ...
  map.forEachFeatureAtPixel(evt.pixel, function(feature, layer) {
    ...
    if (geom.getLength) {
      ...
      geom.transform(ol.proj.getTransform(projCode, 'EPSG:4326'));
      var coords = geom.getCoordinates();
      var length = 0.0;
      for (var i = 1; i < coords.length; i++) {
        length += ol.sphere.NORMAL.haversineDistance(coords[i-1], coords[i]);
      }
