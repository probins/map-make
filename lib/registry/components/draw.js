...
$('#download').onclick = function(e) {
  ...
  e.target.href = 'data:' + (mongo ? mediaTypes['mongo'] : mediaTypes[formatType])
      + ';charset=utf-8,' + encodeURIComponent($('#serialOP').value);
  var ext = mongo ? 'json' : formatType.toLowerCase();
...
function serialise(saveType, tolerance) {
  ...
  function doWrite(features, reproject, geoonly) {
    ...
    // FIXME would be better to have a custom format for Mongo
    if (mongo) {
      var mongoOut = JSON.parse(out);
      var mOut = [];
      if (geoonly) {
        mOut.push({geometry: mongoOut});
      } else {
        mongoOut.features.forEach(function(f) {
          var feature = {
            id: f.id,
            geometry: f.geometry
          };
  ...
  if (tolerance) {
    ...
    if (geoonly) {
      if (mongo) {
        op = JSON.stringify(JSON.parse(op)[0].geometry);
      }
