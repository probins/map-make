...
var source = vectors.getLayers().item(vectors.getLayers().getLength() - 1).get('source');
...
interactions.ls.on('drawstart', function(evt) {
  ...
  listener = evt.feature.getGeometry().on('change', function(evt) {
    ...
    var length = measure.getLength(evt.target, map.get('view').getProjection());
...
interactions.poly.on('drawstart', function(evt) {
  ...
  listener = evt.feature.getGeometry().on('change', function(evt) {
    ...
    var area = measure.getArea(evt.target, map.get('view').getProjection());
