...
var PRECISION = 8; // coordinate precision (decimal places) used by save
...
function serialise(saveType, tolerance) {
  ...
  var serFeatures = format.readFeatures(format.writeFeatures(outFeatures, {
  ...
  serFeatures.forEach(function(f) {
    ...
    function parse(coords) {
      return [
        parseFloat(coords[0].toPrecision(PRECISION)),
        parseFloat(coords[1].toPrecision(PRECISION))
      ];
    }
    // FIXME only handling points/linestrings/polygons
    if (geom instanceof ol.geom.Point) {
      coords = parse(coords);
    }
    if (geom instanceof ol.geom.LineString) {
      coords.forEach(function(c, i, a) {
        a[i] = parse(c);
      });
    }
    if (geom instanceof ol.geom.Polygon) {
      // FIXME only operating on 1st coords array
      coords[0].forEach(function(c, i, a) {
        a[i] = parse(c);
      });
    }
  ...
  var geoonly = $('#geoonly').checked && formatType == 'GeoJSON' &&
      serFeatures.length === 1;
  $('#serialOP').value = geoonly ? format.writeGeometry(serFeatures[0].getGeometry()) :
      format.writeFeatures(serFeatures);
}
