...
// fetch is relative to html page, so find list.json relative to current module
const url = new URL(import.meta.url);
let elems = url.pathname.split('/');
elems.pop();
elems.pop();
let listUrl = `${elems.join('/')}/sources/list.json`;
...
$('#addraster').addEventListener('click', function() {
  ...
  .then(function(m) {
    ...
    var id = layers.item(layers.get('length') - 1).getProperties().id;
