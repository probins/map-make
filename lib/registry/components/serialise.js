...
$('#serialise-title').addEventListener('click', () => {
...
function serialise(saveType, id, tolerance) {
  ...
  let format = vectors.getFormat(formatType);
  let layers = vectors.getLayers().getArray();
  let outFeatures = [];
  layers.forEach(l => {
  ...
  let geoonly = $('#geoonly').checked && formatType == 'GeoJSON' &&
  ...
  if (tolerance) {
    ...
    if (geoonly) {
      let feats = [new Feature(format.readGeometry(op).simplify(tolerance))];
