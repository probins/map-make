...
function switcherHandler(evt) {
  ...
  // if radio button group, get layers for this group
  if (newLayer.get('group')) {
    const groupLayers = this.getLayers().item(1).get('layers').getArray().filter(function(l) {
      return l.get('group') == newLayer.get('group');
    });
