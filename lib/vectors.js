 ...
 * - add() which uses OL3 code to fetch a vector source and add to map;
 *   it also handles data from a MongoDB, using fetch() and formatting as appropriate;
  ...
  options.vectors.forEach(function(vector) {
    ...
    if (rtns[1]) {
      ...
      if (comps.indexOf('popup') == -1) {
        $('#featuredisplaybutton').style.display = 'block';
      }
