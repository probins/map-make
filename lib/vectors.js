...
function switcherHandler(evt) {
  ...
  // 'this' is map
  let newLayer = this.getLayers().item(1).get('layers').getArray().filter(function(l) {
  ...
  options.vectors.forEach(vector => {
    ...
    if (vector.file) {
      ...
      sourceOptions.loader = (extent, resolution, projection) => {
        if (vector.strategy && vector.strategy == 'bbox') {
          let wgsExt = transformExtent(extent, projection, 'EPSG:4326');
        ...
        .then(result => {
          let features = sourceOptions.format.readFeatures(result, readOptions);
          let layers = getLayers().getArray().filter(function(l) {
            return l.getProperties().id == vector.id;
          });
          let source = layers[0].get('source');
          source.addFeatures(features);
        })
        .catch(() => utils.errors.fetchFail());
      };
    }
    let source = new VectorSource(sourceOptions);
    ...
    let newLayer = new VectorLayer(vectOpts);
    ...
    let rtns = switcher.addVectorDiv(vector);
...
function getStyle(type, feature) {
  ...
  strokeAttrs.forEach(sa => {
    if (Array.isArray(stroke[sa])) {
      ...
      let attr = feature.get(stroke[sa][0]);
...
function addFeaturesListener() {
  let vectorLayers = getLayers().getArray();
  // copied from extent.createEmpty()
  let vectorsExtent = [Infinity, Infinity, -Infinity, -Infinity];
  let sourcesRead = 0;
  // callback
  let callback = e => {
    // copied from extent.extend
    let extend = (extent1, extent2) => {
