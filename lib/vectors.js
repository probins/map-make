 ...
 * needs ol, getVectorLayers, map, getStyle, switcher, switcherHandler, addComponentButtons
 */
  ...
  options.vectors.forEach(function(vector) {
    ...
    if (vector.type == 'mongodb') {
        ...
        fetch(url).then(function(response) {
          ...
          result.forEach(function(r) {
            ...
            for (var key in r) {
              if (key == 'id') {
                feature.setId(r[key]);
              } else if (r[key].type && r[key].coordinates) {
      ...
      if (url && !vector.format) {
        ...
        format = getFormatForExtension(url.substring(url.lastIndexOf('.')+1));
      }
    ...
    // if this is 1st vector layer, add vector component buttons
    if (rtns[1]) {
      addComponentButtons(options.components);
    }
  });
}
...
function addComponentButtons(components) {
  ...
  if (!components.popup) {
    ...
    document.getElementById('components-content').appendChild(button);
  }
